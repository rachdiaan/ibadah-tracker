...
function updateCountdown() {
    ...
    const todayPrayerData = getPrayerTimes(new Date());
    prayerTimesContainer.innerHTML = '';
    Object.entries(todayPrayerData).forEach(([key, value]) => {
        const card = document.createElement('div');
        card.className = 'prayer-card';
        if (key === nextPrayer.key && (nextPrayer.time - new Date() > 0)) {
    ...
    if (diff > 0) {
        ...
        countdownTimerEl.textContent = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
         ...
         if (lastPrayerAlertTime !== nextPrayer.time.getTime()) {
            showPrayerTimeAlert(nextPrayer, false);
            lastPrayerAlertTime = nextPrayer.time.getTime();
         }
    }
}
